// scripts/sendWeeklyReports.js
//
// Sends the Weekly Report to every store's configured recipients.
// Meant to run automatically via a cron job every Monday night — see
// the deployment notes for exactly how to schedule it.
//
...
// anyone else.
//
//   node scripts/sendWeeklyReports.js
//   node scripts/sendWeeklyReports.js --test=you@example.com

require("dotenv").config();
const pool = require("../src/db");
const { getWeeklyReportData } = require("../src/reportData");
const { renderWeeklyReportEmail } = require("../src/emailTemplates/weeklyReport");
...
async function main() {
  ...
  for (const store of stores) {
    try {
      ...
      const report = await getWeeklyReportData(store.id);
      const html = renderWeeklyReportEmail(store.name, report);
      const subject = `${store.name} — Weekly Report (${report.weekStartDisplay} – ${report.weekEndDisplay})`;
...
main().catch((err) => {
  console.error("Fatal error running sendWeeklyReports:", err);
  process.exit(1);
});
